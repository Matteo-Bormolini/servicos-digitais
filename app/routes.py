...
def cadastrar_cpf():
    form_cpf = FormCadastroCPF()
    if form_cpf.validate_on_submit():
        email = form_cpf.email.data.strip().lower()
        cpf_digits = apenas_numeros(form_cpf.cpf.data)
        ...
        senha_hash = bcrypt.generate_password_hash(form_cpf.senha.data).decode('utf-8')

        novo = ClienteCPF(
            nome=form_cpf.username.data,
            telefone=form_cpf.telefone.data or None,
            email=email,
        ...
        # --- salvar foto ---
        saved_name = None
        if getattr(form_cpf, 'foto_perfil', None) and form_cpf.foto_perfil.data:
            try:
                saved_name, _thumb = salvar_imagem(
                    form_cpf.foto_perfil.data,
                    folder='perfil',
                    prefix='user',
                    gerar_thumb=False
                )
    ...
    return render_template('cadastro_cliente.html', form_cpf=form_cpf)
...
def cadastrar_cnpj():
    ...
    if form_cnpj.validate_on_submit():
        ...
        return redirect(url_for('login', show_edit_prompt=1))
    return render_template('cadastro_cnpj.html', form_cnpj=form_cnpj)
...
def cadastrar_prestador():
    ...
    """
    FormCadastroPrestador = FormCadastroPrestador()

    if FormCadastroPrestador.validate_on_submit():
        email = FormCadastroPrestador.email.data.strip().lower()
        cnpj_digits = apenas_numeros(FormCadastroPrestador.cnpj.data)
        ...
        # gerar senha
        senha_hash = bcrypt.generate_password_hash(FormCadastroPrestador.senha.data).decode('utf-8')

        # Cria objeto Prestador
        novo = PrestadorServico(
            nome = FormCadastroPrestador.username.data if hasattr(FormCadastroPrestador, 'username') else (FormCadastroPrestador.nome.data if hasattr(FormCadastroPrestador, 'nome') else None),
            telefone = (FormCadastroPrestador.telefone.data or None) if hasattr(FormCadastroPrestador, 'telefone') else None,
            email = email,
            senha_hash = senha_hash,
            cnpj = cnpj_digits,
            especialidade = FormCadastroPrestador.especialidade.data if hasattr(FormCadastroPrestador, 'especialidade') else None,
            tipo = 'prestador',
        ...
        # --- salvar foto ---
        nome_salvo = None
        if getattr(FormCadastroPrestador, 'foto_perfil', None) and FormCadastroPrestador.foto_perfil.data:
            try:
                nome_salvo, _ = salvar_imagem(
                    FormCadastroPrestador.foto_perfil.data,
                    folder='perfil',
                    prefix='prestador',
                    gerar_thumb=False
                )
        ...
        return redirect(url_for('home'))
    return render_template('cadastrar_prestador.html', FormCadastroPrestador=FormCadastroPrestador)
